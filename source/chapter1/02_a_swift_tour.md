...
按照传统，学习编程语言的第一件事就是在屏幕上打印“Hello, world”。在 `Swift`中，可以用一行代码实现：

    println("Hello, world")

如果你使用过`C`或者`Objective-C`，那你应该很熟悉这种语法。不过在`Swift`中，这行代码就是一个完整的程序。你不需要为了诸如I/O或者字符处理这样的功能导入一个单独的库。全局作用域中的代码会被自动当做程序的入口点，所以你也不需要`main`函数。你同样不需要在每个语句结尾写上分号。
...
使用`let`来声明常量，使用`var`来声明变量。常量在编译的时候并不需要有明确的值，但是你只能为它赋值一次。也就是说你可以用常量来表示这样一个值：你只需要决定一次，但是需要使用很多次。

    var myVariable = 42
    myVariable = 50
    let myConstant = 42

常量或者变量的类型必须和你赋给它们的值一样。然而，声明时类型是可选的，声明的同时赋值的话，编译器会自动推断类型。在上面的例子中，由于`myVariable`的初始值是整数，所以编译器推断出它是一个整数（integer）。
...
你可以一起使用`if`和`let`来处理值缺失的情况。有些变量的值是可选的。可选的值可能是一个具体的值或者是`nil`，`nil`表示值缺失。在类型后面加一个问号来标记这个变量的值是可选的。
...
`switch`支持任意类型数据(不仅仅是整数)的各种比较操作以及测试相等。
...
你可以在循环中指定一个索引并使用`..<`来表示范围，也可以使用传统的写法，两者是等价的：
...
使用`func`来声明一个函数，使用名字和参数来调用函数。使用`->`来指定函数返回类型。
...
使用元组表示复合值，例如一个方法要返回的多个值。元组中的元素可以按名称或索引引用：
...
函数实际上是一种特殊的闭包：代码块将会在以后调用。闭包中的代码可以访问创建闭包的域中的变量和方法，即使执行闭包的是另一个域。你可以使用`{}`来创建一个匿名闭包。使用`in`将参数和返回值类型声明与闭包函数体进行分离。
...
在类名后面加上括号来创建类的一个实例。使用点语法来访问实例的属性和方法。

    var shape = Shape()
    shape.numberOfSides = 7
    var shapeDescription = shape.simpleDescription()

这个版本的`Shape`类缺少了一些重要的东西：一个用来初始化类实例的构造函数。使用`init`来创建一个构造器。
...
注意`self`是怎么被用来区别属性`name`和构造器参数`name`的。当你创建实例的时候，像传入函数参数一样给类传入构造器的参数。每个属性都需要赋值——无论是通过声明（就像`numberOfSides`）还是通过构造器（就像`name`）。
...
如果你不需要计算属性，但是需要在设置一个新值之前或者之后运行代码，使用`willSet`和`didSet`。
...
枚举的成员值是实际值，并不是原始值的另一种表达方法。实际上如果原始值没有意义，就不需要设置。
...
注意声明`SimpleStructure`的时候`mutating`关键字用来标记一个会修改结构体的方法。`SimpleClass`的声明不需要标记任何方法，因为类中的方法经常会修改类。

使用`extension`来为现有的类型添加功能，比如新的方法和参数。可以使用扩展为任意地方声明的类型添加协议一致性，甚至是从外部库或者框架引入的一个类型，使得这个类型遵循某个协议。
...
你可以像使用其他命名类型一样使用协议名——例如，可以创建一个有不同类型但是都实现一个协议的对象集合。当你处理类型是协议的值时，协议外定义的方法不可用。
