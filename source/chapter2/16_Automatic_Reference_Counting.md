...
> 注意：  
> `CreditCard`类的`number`属性被定义为`UInt64`类型而不是`Int`类型，是为了确保`number`属性在32位和64位系统上都足以存储一个16位的卡号。
...
循环强引用还会发生在将一个闭包赋值给类实例的某个属性，并且这个闭包体中又使用了该实例的情况。这个闭包体中可能访问了实例的某个属性，例如`self.someProperty`，或者闭包中调用了实例的某个方法，例如`self.someMethod`。这两种情况都导致了闭包 “捕获" `self`，从而产生了循环强引用。
