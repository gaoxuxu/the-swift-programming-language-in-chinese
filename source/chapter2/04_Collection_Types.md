...
如果创建一个数组或者字典并将其赋给一个变量，这个集合就是可变的。这意味着你可以在创建集合后对其进行修改，添加、移除，或者改变集合中存储的项。相反地，如果你将一个数组或者字段赋给一个常量，那这个数组或者常量就是不可变的，他的大小和内容都不能改变。
...
Swift 数组指定了它所存储元素的类型。这与 Objective-C 的 NSArray 和 NSMutableArray 不同，这两个类可以存储任意类型的对象，并且不提供所返回对象的任何特别信息。在 Swift 中，数据值在被存储进入某个数组之前类型必须明确，方法是通过显式的类型标注或类型推断，而且不是必须是`class`类型。例如：如果我们创建了一个`Int`值类型的数组，我们不能往其中插入任何不是`Int`类型的数据。 Swift 中的数组是类型安全的，并且它们中包含的类型必须明确。

<a name="array_type_shorthand_syntax"></a>
### 数组的简单语法

写 Swift 数组应该遵循像`Array<SomeType>`这样的形式，其中`SomeType`是这个数组中唯一允许存在的数据类型。也可以使用像`[SomeType]`这样的简单语法。 尽管两种形式在功能上是一样的，但是推荐较短的那种，而且在本文中都会使用这种形式来使用数组。
...
下面这个例子创建了一个叫做`shoppingList`并且存储`String`值的数组：

    var shoppingList: String[] = ["Eggs", "Milk"]
    // shoppingList 已经被构造并且拥有两个初始项。

`shoppingList`变量被声明为“一个`String`值的数组“，记作`[String]`。 因为这个数组被规定只有`String`一种数据结构，所以只有`String`类型可以在其中被存取。 在这里，`shoppinglist`数组由两个`String`值（`"Eggs"` 和`"Milk"`）构造，并且由字面量定义。
...
调用数组的`insert(atIndex:)`方法来在某个具体索引值插入数据项：
...
注意，由于`someInts`被设置为一个`[Int]`构造器的输出，所以它的类型被断定为`[Int]`。
...
Swift 中的`Array`类型还提供一个可以创建特定大小并且所有数据都被默认的构造方法。我们可以把准备加入新数组的数据项数量（`count`）和适当类型的初始值（`repeatedValue`）传入数组构造函数：
    ...
    // threeDoubles类型为[Double], 等于 [0.0, 0.0, 0.0]
...
字典是一种存储多个相同类型的值的容器。每个值（value）都关联唯一的键（key），键作为字典中的这个值的标识符。和数组中的数据项不同，字典中的数据项并没有具体顺序。我们在需要通过标识符（键）访问数据的时候使用字典，这种方法很大程度上和我们在现实世界中使用字典查字义的方法一样。

> 注意：  
> Swift 的字典使用时需要具体规定可以存储键和值类型。不同于 Objective-C 的`NSDictionary`和`NSMutableDictionary` 类，它们可以使用任何类型的对象来作键和值并且不提供任何关于这些对象的本质信息。在 Swift 中，在某个特定字典中可以存储的键和值必须提前定义清楚，方法是通过显性类型标注或者类型推断。

<a name="dicionary_type_shorthand_syntax"></a>
###字典的简单语法

Swift 的字典使用`Dictionary<KeyType, ValueType>`定义,其中`KeyType`是字典中键的数据类型，`ValueType`是字典中对应于这些键所存储值的数据类型。

也可以使用字典类型的简单形式`[KeyType: ValueType]`。尽管两种形式功能相同，但是简单语法更好，本文中以后都会使用简单形式。
...
因为这个语句中所有的键和值都分别是相同的数据类型，Swift 可以推断出`［String: String]`是`airports`字典的正确类型。
...
使用一个布尔属性`isEmpty`来检查`count`属性是否为0:
...
<a name="hash_values_for_dictionary_key_types"></a>
###字典键类型的哈希值

字典的键的类型必须是可哈希的，也就是说，键的类型必须提供一种可以计算自身哈希值的途径。哈希值是一个`Int`类型的值，相同的对象具有相同的哈希值，假如`a == b`，必然也就有`a.hashValue == b.hashValue`。

Swift的所有基本类型（比如`String`、`Int`、`Double`和`Bool`）都默认是可哈希的，这些类型都可以被用作字典的键。没有关联值的枚举成员值也默认是可哈希的。

> 注意：  
> 你可以使用自定义的遵守Swift标准库中`Hashable`协议的类型作为字典的键类型。实现了`Hashable`协议的类型必须提供一个名为`hashValue`、返回值类型为`Int`的`getter`，并且必须提供一个相等操作符`==`的实现。在同一个程序的不同实例或者不同的程序中，`hashValue`返回的值可以不相同。
> 更多关于协议的内容，请参见[协议](https://developer.apple.com/library/prerelease/ios/documentation/Swift/Conceptual/Swift_Programming_Language/Protocols.html#//apple_ref/doc/uid/TP40014097-CH25-XID_390)