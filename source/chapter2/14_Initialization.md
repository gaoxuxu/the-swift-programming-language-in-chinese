...
`Color`提供了一个构造器，其中包含三个`Double`类型的构造参数：`Color`还提供一个只有一个参数`white`的构造器，这个构造器使三个参数拥有相同值：
...
两个构造器都可以被用来初始化`Color`实例，只需要提供每个构造器参数的值：
...
注意，如果不通过外部参数名字传值，你是没法调用这个构造器的。只要构造器定义了某个外部参数名，你就必须使用它，忽略它将导致编译错误：
    ...
    // 报编译时错误，external names are required
    
###没有外部名称的构造器参数

如果不想对一个构造器参数使用外部名称，可以使用一个下划线（`_`）代替显式指定外部名称来重写该默认行为。

下面是`Celsius`的一个扩展版本，该版本添加了从一个`Double`值创建`Celsius`类实例的构造器，该值表示一个已经存在的摄氏温度：
...
构造器调用方式`Celsius(37.0)`的意图非常明显，因此不需要外部参数名称。
...
只要在构造过程结束前常量的值能确定，就可以在构造过程中的任意时间点修改常量属性的值。
...
类的指定构造器的书写方式和值类型的简单构造器差不多：

    init(parameters) {
        statements
    }

便利构造器的书写方式差不多，只不过在`init`关键字前多了个用空格分隔的`convenience`修饰符：
...
> 注意：  
> 这些规则不会影响使用时如何用类去创建实例。任何上图中展示的构造器都可以用来完整创建对应类的实例。这些规则只在实现类的定义时有影响。
...
> 注意：  
> 特定的形势下可以继承父类的构造器，但仅限于能够保证安全并且这种做法是合适的。更多信息，请见下面的自动构造器的继承一节。

假如你希望自定义的子类中能实现一个或多个跟父类相同的构造器--也许是为了完成一些定制的构造过程--你可以在你定制的子类中提供和重载与父类相同的构造器。

如果你重载的构造器是一个指定构造器，你可以在子类里重载它的实现。此时，你必须在子类构造器定义前加上`override`修饰符。即使是重写一个自动提供的默认构造器，这样也是可行的。默认构造器参见[默认构造器](#default_initializers)

与重载属性、方法或者下标脚本一样，修饰符`override`会让Swift检查父类中是否有匹配的要被重载的构造器，并且使子类重写的构造器的参数生效。

> 注意：  
> 重载父类的指定构造器时，应该总是写上`override`关键字，即使子类的实现是一个便利构造器。

与此相反，由于父类的便利构造器不会被子类直接调用（见[构造链](#initialization_chain)），所以如果子类构造器匹配到的是父类的便利构造器的话，严格来说子类就不会重载父类的构造器。也因此，子类构造器匹配的是父类的便利构造器时，也就不用写`override`修饰符了。

下面的例子定义了一个名为`Vehicle`的基类。这个基类声明了一个名为`numberOfWheels`的存储型属性，其默认值为`Int`值`0`。计算型属性`description`使用`numbersOfWheels`属性来创建一个描述车辆特征的`String`：
...
`Vehicle`类给它唯一的存储型变量提供了一个默认值，并且其自身没有任何的自定义的构造器。根据[默认构造器](#default_initializers)中的描述，它将自动获得一个默认构造器。这个获得的默认构造器对类来说始终是指定构造器，它可以被用来创建一个`numberOfWheels`的值为`0`的`Vehicle`实例：

    let vehicle = Vehicle()
    println("Vehicle: \(vehicle.description)")
    // Vehicle: 0 wheel(s)

下面这个例子定义了一个名为`Bicycle`的`Vehicle`的子类：
...
`Bicycle`类自定义了指定构造器`init()`。这个指定构造器匹配到了`Bicycle`的父类的指定构造器，因此`Bicycle`版本的这个构造器需要以修饰符`override`修饰。

`Bicycle`的`init()`构造器以调用`super.init()`开始，也就是调用`Bicycle`类的父类`Vehicle`类的默认构造器。这样做确保了继承得到的属性`numberOfWheels`被`Vehicle`类先于`Bicycle`类初始化，不会影响到后者对该属性的修改。调用`super.init()`之后，`numberOfWheels`的原始值被重置为新的值`2`。

如果你创建一个`Bicycle`类的实例，就可以调用它继承的计算型属性`description`来看看`numberOfWheels`属性被更新过后的值：
...
> 注意：  
> 父类只允许子类在初始化期间修改变量属性的值。不能修改继承自父类的常量属性。
...
`RecipeIngredient`类提供的便利构造器`init(name: String)`和`Food`类提供的指定构造器`init(name: String)`有相同的参数。由于这个便利构造器重载了父类的指定构造器，所以它必须加上`override`修饰符（参见[构造器的继承和重载](#initializer_inheritance_and_overriding)）。

尽管`RecipeIngredient`这个构造器是便利构造器，`RecipeIngredient`依然提供了对所有父类指定构造器的实现。因此，`RecipeIngredient`也能自动继承了所有父类的便利构造器。
...
在类的构造器定义前加上`required`关键字来标明该类的所有子类都必需实现这个构造器：
...
应该在所有子类实现的必需构造器前写上`required`修饰符，以此来表明这个构造器需要应用到该构造器链中的所有子类中。重写必需的指定构造器时，不需要再写上`override`修饰符：
...
> 注意：  
> 如果继承得到的必需构造器已经可以满足当前场景，也可以不用给该必需构造器提供明确的实现。
