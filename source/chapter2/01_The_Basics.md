...
就像 C 语言一样，Swift 使用变量来进行存储并通过变量名来引用值。在 Swift 中，广泛的使用着值不可变的变量，它们就是常量，而且比 C 语言的常量更强大。在 Swift 中，如果你要处理的值不需要改变，那使用常量可以让你的代码更加安全并且更清晰地表达你的意图。

除了我们熟悉的类型，Swift 还增加了 Objective-C 中没有的高阶数据类型比如元组（Tuple）。元组可以让你创建或者传递一组数据，比如作为函数的返回值时，你可以用元组作为一个复合值返回多个值。
...
你可以在一行定义多个类型相同的变量：各个变量之间用逗号分隔，在最后一个变量名后添加类型标注即可：
...
常量与变量名不能包含数学符号、箭头、保留的（或者非法的）Unicode 码位、连线以及制表符。不能以数字开头，但是可以在常量与变量名的其他地方包含数字。
...
Swift 是一门_类型安全（type safe）_的语言。类型安全的语言可以让你清楚地知道代码要处理的值的类型。如果你的代码需要一个`String`，你绝对不可能不小心传进去一个`Int`。
...
Swift 有一个基本的_布尔（Boolean）_类型，叫做`Bool`。布尔值指_合乎逻辑的（logical）_，因为它们只能是真或者假。Swift 有两个布尔常量，`true`和`false`：
...
你可以给可选变量赋值为`nil`来表示它没有值：
    ...
    // serverResponseCode 包含一个实际的Int值404
...
你可以通过使用比较一个可选的值与`nil`的`if`语句来判断一个可选是否包含值。条件语句中的等于操作符为`==`，不等于操作符为`!=`。  
如果一个可选有值，那它就被认为“不等于”`nil`：
...
一旦你确定可选包含一个值，你就可以通过在可选的名字后面添加一个感叹号（`!`）的方式访问它的值。感叹号的意思是，“我知道这个可选是有值的，请使用它。”这相当于给可选的值强制解包：
...
你可以像上面这样使用可选绑定代替强制解包来重写`possibleNumber`这个例子：
